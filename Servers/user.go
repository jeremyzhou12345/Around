...
func checkUser(username, password string) bool {
	...
	queryResult, err := es_client.Search().
		Index(INDEX). //index is our database
		Query(termQuery).
		Pretty(true).
		Do()
	if err != nil {
...
func addUser(user User) bool {
	...
	//check if there already exists the User
	...
	if queryResult.TotalHits() > 0 { //user exists
		fmt.Printf("User %s already exists, cannot create duplicate user.\n", user.Username)
		return false
	}

	//save User into Index of ES
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "text/plain")       //return are all text
	w.Header().Set("Access-Control-Allow-Origin", "*") //everyone can access
}
